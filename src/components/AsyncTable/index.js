...
export default class TableExpand extends React.Component {
  ...
  //返回的数据生成所需的数组
	getResultToArray = (typeProps = 'data',res,isEditId) =>{
		...
		if(isEditId){
			data = data.map((v,i)=>{return {id:i,...v}})
		}
		return data || res;
	}
	//根据头部详情获取数据
				...
				case 'number':
					if(this.isArray(s.values)) {
						...
						if(s.values[0]!==null && s.values[1]!==null) {
							where[s.field] = {
								between: [s.values[0] * ratio, s.values[1] * ratio]
							};
  ...
  jumpUrl = (newParams = {}) => {
    ...
    if(newParams.searchs){
    	...
    	const params = Object.assign({}, this.params,newParams);
			...
			this.props.replace(path,`?q=${encodeURIComponent(JSON.stringify(params))}`);
    	this.forSearchList()
    	return;
    }
...
//切换时间显示和金钱显示
  formatValue = (type, value) => {
	...
	returnRender = (columns,search)=>{
		const { api,uuid, buttons, pages ,isTabs,defaultKey,TabsData,path,expandedRowRender,resetFresh,getArea,hasInitValue } = this.props;
